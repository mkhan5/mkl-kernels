...
int main(int argc, char **argv)
{
   ...
   */
   ...
   /*
    ...
    D = (double *)mkl_malloc( n*n*sizeof( double ), 64 );
    E = (double *)mkl_malloc( n*n*sizeof( double ), 64 );
...
//  float tmpb[3][3] = { 1, 0, 1, 3, -1, -2, 0, 2, -1};
    double tmpa[3][3] = { 24, -22, -4, -11, 12, 10, -23, -18, -12};
    double tmpb[3][3] = { 20, -25, -1, -2, -3, 14, -24, -9, -19};
    double tmpc[3][3] = {-17, -16, 19, 8, 5, 4, 17, -13, 22};
    double tmpd[3][3] = {-5, -8, -6, 1, -15, 23, 3, -21, 11};
  ...
  //  x[3] = { 2, 1, 3};
   ...
   // }
    ...
    tmp_2d_array_AB = (double *)mkl_malloc( n*n*sizeof( double ), 64 );
    tmp_2d_array_CD = (double *)mkl_malloc( n*n*sizeof( double ), 64 );
    cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, m, n, p, alpha, A, p, B, n, beta, tmp_2d_array_AB, n);
    cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, m, n, p, alpha, C, p, D, n, beta, tmp_2d_array_CD, n);
    cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, m, n, p, alpha, tmp_2d_array_AB, p, tmp_2d_array_CD, n, beta, E, n);
	...
	if(debug)
	{
	    ...
	    for (i=0; i<min(m,6); i++) {
	      for (j=0; j<min(n,6); j++) {
		printf ("%12.0f", D[j+i*n]);
	      }
	      printf ("\n");

	    }

	    printf ("\n Top left corner of matrix Res E: \n");
	    for (i=0; i<min(m,6); i++) {
	      for (j=0; j<min(n,6); j++) {
		printf ("%12.0f", E[j+i*n]);
	      }
	      printf ("\n");
