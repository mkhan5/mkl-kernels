...
int main(int argc, char **argv)
{
   ...
   */
   ...
   /*
    ...
    q = (double *)mkl_malloc( n*sizeof( double ), 64 );
   ...
   // float tmpx[3] = { 13, 31, 49};
    ...
    float tmpa[3][3] = { 1, 2, 8, 10, 3, 9, 7, 4, 17};
    float tmpp[3] =  {21, -13, -11};
    float tmpr[3] = { -12, -3, 7};
  ...
  //  x[3] = { 2, 1, 3};
   ...
   // }
    ...
    cblas_dgemv(CblasRowMajor, CblasNoTrans, m, n, alpha, A, m, p, 1, beta, q, 1);
    cblas_dgemv(CblasRowMajor, CblasTrans, m, n, alpha, A, m, r, 1, beta, s, 1);
  ...
  //  cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, scale_100, n, p, alpha, A, p, B, n, beta, C, n);
	...
	if(debug)
	{
	    ...
	    printf ("\n Top left corner of matrix S: \n");
	    for (i=0; i<min(m,6); i++) {
	      for (j=0; j<min(n,6); j++) {
		printf ("%12.0f", s[j+i*n]);
	      }
	      printf ("\n");
	    }

	    printf ("\n Top left corner of matrix Q: \n");
	    for (i=0; i<min(m,6); i++) {
	      for (j=0; j<min(n,6); j++) {
		printf ("%12.0f", q[j+i*n]);
	      }
	      printf ("\n");
	    }
	}
